...
eqv [l1@(List arg1), l2@(List arg2)] = eqvList equal [l1, l2]
eqv [(DottedList xs x), (DottedList ys y)] = eqvList equal [ (List $ xs ++ [x]), (List $ ys ++ [y])]
eqv [arg1, arg2] = do
...
stringRef [(String s), (Number k)]
  | length s < fromIntegral k + 1 = throwError $ Default "out of bound error"
  | otherwise = Right $ Character $ s !! fromIntegral k
...
bindVars envRefbindings = readIORef envRef >>= extendEnv
bindings >>= newIORef
